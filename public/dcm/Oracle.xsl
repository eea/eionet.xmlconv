...
<!--   TEMPLATE match ="Row"  -->
   ...
   <!--   TEMPLATE name="create-table"  -->
        ...
        <xsl:element name="xsl:for-each" namespace="http://www.w3.org/1999/XSL/Transform">
            <xsl:attribute name="select">$elementsMetadata/element</xsl:attribute>
            <xsl:text disable-output-escaping="yes">  &lt;xsl:if test="string-length(identifier)     </xsl:text>
            <xsl:text disable-output-escaping="no">    &gt;     </xsl:text>
            <xsl:text disable-output-escaping="yes">   30"&gt;    </xsl:text>
            <xsl:text disable-output-escaping="yes">   "</xsl:text>
            <xsl:text disable-output-escaping="yes">&lt;xsl:value-of select="substring(identifier,0,28)" /&gt; </xsl:text>
            ...
            <xsl:text disable-output-escaping="yes">  "</xsl:text>
            <xsl:text disable-output-escaping="yes">&lt;xsl:value-of select="identifier" /&gt;</xsl:text>
            <xsl:text disable-output-escaping="yes">"</xsl:text>
            <xsl:text disable-output-escaping="yes">  &lt;/xsl:if&gt;</xsl:text>
