...
final class XMLScanner extends Job {
    ...
    private org.basex.build.xml.XMLScanner.Scan scan = org.basex.build.xml.XMLScanner.Scan.CONTENT;
    ...
    private void scanCONTENT(final int ch) throws IOException {
        ...
        prolog = false;
        scan = org.basex.build.xml.XMLScanner.Scan.ELEMENT;
    ...
    private void scanELEMENT(final int ch) throws IOException {
        ...
        if(c == '>') {
            type = Type.R_BR;
            scan = org.basex.build.xml.XMLScanner.Scan.CONTENT;
        } else if(c == '=') {
            // scan equal sign...
            type = Type.EQ;
        } else if(c == '\'' || c == '"') {
            // scan quote...
            type = Type.QUOTE;
            scan = org.basex.build.xml.XMLScanner.Scan.QUOTE;
            quote = c;
        } else if(c == '/') {
            ...
            type = scan == org.basex.build.xml.XMLScanner.Scan.ATT ? Type.ATTNAME : Type.ELEMNAME;
            do token.add(c); while(isChar(c = nextChar()));
            prev(1);
            scan = org.basex.build.xml.XMLScanner.Scan.ATT;
        } else {
            // undefined character...
