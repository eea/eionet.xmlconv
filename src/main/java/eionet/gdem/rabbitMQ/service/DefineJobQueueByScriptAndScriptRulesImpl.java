...
public class DefineJobQueueByScriptAndScriptRulesImpl implements DefineJobQueueAndSendToRabbitMQTemplate {
    ...
    private RabbitMQMessageSender rabbitMQSyncFmeMessageSender;
    private RabbitMQMessageSender rabbitMQAsyncFmeMessageSender;
    ...
    public DefineJobQueueByScriptAndScriptRulesImpl(WorkerAndJobStatusHandlerService workerAndJobStatusHandlerService, JobService jobService,
                                                    ...
                                                    @Qualifier("syncFmeJobRabbitMessageSenderImpl") RabbitMQMessageSender rabbitMQSyncFmeMessageSender,
                                                    @Qualifier("asyncFmeJobRabbitMessageSenderImpl") RabbitMQMessageSender rabbitMQAsyncFmeMessageSender) {
    ...
    public void checkRules(QueryEntry queryEntry, JobEntry jobEntry) throws XMLConvException {
        ...
        if (!jobEntry.isHeavy()) {
            ...
            if (queryEntry.getRuleMatch()!=null && scriptRulesEntries.size()>0 && queryEntry.getRuleMatch().equals(ScriptRuleMatch.ALL.getValue())) {
            ...
            } else if (queryEntry.getRuleMatch()!=null && scriptRulesEntries.size()>0 && queryEntry.getRuleMatch().equals(ScriptRuleMatch.AT_LEAST_ONE.getValue())) {
                for (ScriptRulesEntry rule : scriptRulesEntries) {
