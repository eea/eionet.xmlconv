...
public class ConvTypeMySqlDao extends MySqlBaseDao implements IConvTypeDao {
     ...
     *
     * String sql = "SELECT " + XSL_TABLE + "." + CNV_ID_FLD + "," + XSL_TABLE + "." + XSL_FILE_FLD + ", " + XSL_TABLE + "." +
     * DESCR_FLD + "," + RESULT_TYPE_FLD + ", " + SCHEMA_TABLE + "." + XML_SCHEMA_FLD + ", " + CONVTYPE_TABLE + "." +
     * CONTENT_TYPE_FLD + " FROM " + XSL_TABLE + " LEFT JOIN " + SCHEMA_TABLE + " ON " + XSL_TABLE + "." + XSL_SCHEMA_ID_FLD + "=" +
     * SCHEMA_TABLE + "." + SCHEMA_ID_FLD + " LEFT JOIN " + CONVTYPE_TABLE + " ON " + XSL_TABLE + "." + RESULT_TYPE_FLD + "=" +
     ...
     *
     * // System.out.println(sql); String[][] r = _executeStringQuery(sql);
     *
     * Vector v = new Vector();
     *
     * for (int i = 0; i < r.length; i++) { Hashtable h = new Hashtable(); h.put("convert_id", r[i][0]); h.put("xsl", r[i][1]);
     * h.put("description", r[i][2]); h.put("content_type_out", r[i][5]); h.put("xml_schema", r[i][4]); h.put("result_type",
     * r[i][3]); v.add(h); }
     *
     * return v;
     *
     ...
     */@Override
     ...
     /*
      * public Vector getConvTypes() throws SQLException { String sql = "SELECT " + CONV_TYPE_FLD + ", " + CONTENT_TYPE_FLD + ", " +
      * FILE_EXT_FLD + ", " + CONVTYPE_DESCRIPTION_FLD + " FROM " + CONVTYPE_TABLE + " ORDER BY " + CONV_TYPE_FLD;
      *
      * String r[][] = _executeStringQuery(sql);
      *
      * Vector v = new Vector();
      *
      * for (int i = 0; i < r.length; i++) { Hashtable h = new Hashtable(); h.put("conv_type", r[i][0]); h.put("content_type",
      * r[i][1]); h.put("file_ext", r[i][2]); h.put("description", r[i][3]); v.add(h); } return v; }
      */
     @Override
      ...
      *
      * Hashtable h = new Hashtable(); h.put("conv_type", r[0][0]); h.put("content_type", r[0][1]); h.put("file_ext", r[0][2]);
