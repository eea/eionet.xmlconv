...
public class RootElemMySqlDao extends MySqlBaseDao implements IRootElemDao {
    ...
    private static final String qSchemaRootElems  = "SELECT "
                                                    + ROOTELEM_ID_FLD + ", " + ELEM_NAME_FLD + ", "
                                                    + NAMESPACE_FLD + "," + ELEM_SCHEMA_ID_FLD
                                                    + " FROM " + ROOTELEM_TABLE
                                                    + " WHERE " + ELEM_SCHEMA_ID_FLD + "= ?"
                                                    + " ORDER BY " + ELEM_NAME_FLD;


    private static final String qRootElemMatching  = "SELECT "
                                        + ELEM_SCHEMA_ID_FLD
                                        + " FROM " + ROOTELEM_TABLE
                                        + " WHERE " + ELEM_NAME_FLD + "= ? ";
    ...
    private static final String  qInsertRootElem = "INSERT INTO " + ROOTELEM_TABLE
                                                    + " ( "
                                                    + ELEM_SCHEMA_ID_FLD + ", "
                                                    + ELEM_NAME_FLD + ", "
                                                    + NAMESPACE_FLD
...
*/
    public Vector getSchemaRootElems(String schemaId) throws SQLException{
        ...
        if (isDebugMode){ logger.debug("Query is " + qSchemaRootElems);}
...
*/
    ...
    public Vector getRootElemMatching(String rootElem, String namespace) throws SQLException{
        ...
        try {
            ...
            if (isDebugMode){ logger.debug("Query is " + (hasNamespace?qRootElemMatchingNamespace:qRootElemMatching));}
            rs = pstmt.executeQuery();
            String[][] r = getResults(rs);
...
*/
    ...
    public void removeRootElem(String rootElemId) throws SQLException{
        Connection conn = null;
        PreparedStatement pstmt = null;
        if (isDebugMode){ logger.debug("Query is " + gRemoveRootElem);}
...
*/
    ...
    public String addRootElem(String xmlSchemaID, String elemName, String namespace) throws SQLException{
        ...
        if (isDebugMode){ logger.debug("Query is " + qInsertRootElem);}
        try{
            conn = getConnection();
